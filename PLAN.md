...
### 16.5 Multi-host / NFS homes (owner-directed, 2026-07-18)

Sai's unit of residency is the **host**, not the `$HOME`: tmux sockets are host-local, and the push reflex needs sub-second locality. The shape is *one daemon per host, host-scoped observation state, NFS as the free aggregation layer*:

* **Host-scoped durable state.** `state_dir()` defaults to `~/.local/state/sai/<short-hostname>/` (`events.tsv`, `pings.jsonl`, `out-%N.log`, `daemon.log`). Every file has exactly one writing host and one reading daemon — same host — so NFS append-atomicity, cross-client cache lag, pane-id collisions, and clock skew all disappear structurally. The zsh hook uses `${HOST%%.*}`, the tmux hooks `#{host_short}` (with an mkdir guard for freshly ssh-ed hosts).
* **Host-local volatile state.** `runtime_dir()` (`$SAI_RUNTIME_DIR`, default `/tmp/sai-$UID`, mode 0700, ownership-checked against `/tmp` squatting) holds `status` and `daemon.pid` — the pidfile needs local `O_EXCL` semantics and must never rely on NFS locking. Deliberately independent of `XDG_RUNTIME_DIR`: the owner's environment unsets it in places (VSCode permission quirks), and the tmux server's copy of it can disagree with the shells', which would strand the status file between two paths. `/tmp` is host-local on the fleet, and nothing in it ever needs to cross hosts — everything that aggregates (events, pings) lives in the NFS host dir. Pane logs also stay on the NFS host dir for M0: tmux config cannot resolve env portably at parse time, and a predictable shared `/tmp` path would expose pane content on multi-user machines; `SAI_STATE_DIR` remains the escape hatch if the churn matters.
* **Lazy per-host daemon.** `sai daemon` claims `daemon.pid` atomically (second instance exits; stale pids self-heal). `sai ensure-daemon` is idempotent and is called from the zsh snippet (guarded by a free pidfile-existence check; opt out with `SAI_NO_AUTOSTART=1`), so ssh-ing into any host of the fleet brings its observer up.
* **Aggregation for free.** `sai stats --all-hosts` merges `<state-root>/*/pings.jsonl` — on an NFS home that is the whole fleet with zero transport, collector, or auth. The pull stays host-local (you ask about the machine in front of you); the LLM centralizes via `backend = "http"` pointing every host at the one GPU box.
* **Deliberately not built:** a networked hub/collector service (only justified if the shared-filesystem assumption breaks — then bridge with rsync before inventing a protocol), cross-host push budgets (one human on N hosts ⇒ N budgets is an accepted approximation; tune per-host budget down if the diary complains), and hub-side slow-loop intelligence (M1, §16.4 — when it comes, the return channel is a per-host `inbox.jsonl` written by the hub, keeping the single-writer-per-file discipline).
