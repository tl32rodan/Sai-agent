...
* **Host-local volatile state.** `runtime_dir()` (`$XDG_RUNTIME_DIR/sai`, fallback `/tmp/sai-$UID`, mode 0700) holds `status` and `daemon.pid` — the pidfile needs local `O_EXCL` semantics and must never rely on NFS locking. Pane logs stay on the NFS host dir for M0: tmux config cannot resolve `XDG_RUNTIME_DIR` portably at parse time, and a world-writable `/tmp` path would expose pane content on multi-user machines; `SAI_STATE_DIR` remains the escape hatch if the churn matters.
